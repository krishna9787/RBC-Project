...
public class AddToOrderPage {
	...
	public synchronized void closeClick(String browser) {
		driver = HandleDriver.getDriver(browser);
		if (browser.equalsIgnoreCase("firefox")) {
			Assert.assertEquals(closebutton.isDisplayed(), true);
			Actions a = new Actions(driver);
			a.moveToElement(closebutton).click(closebutton).build().perform(); //Sometimes not working without moveToElement
		} else if (browser.equalsIgnoreCase("chrome")) {
			Assert.assertEquals(continuebutton.isDisplayed(), true);
			Actions a = new Actions(driver);
			a.moveToElement(continuebutton).click(continuebutton).build().perform(); // Sometimes not working without moveToElement
		}
